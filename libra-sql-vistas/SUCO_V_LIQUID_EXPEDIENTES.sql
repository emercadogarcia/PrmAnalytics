  ...
  CREATE OR REPLACE FORCE EDITIONABLE VIEW "CONSULTAS"."SUCO_V_LIQUID_EXPEDIENTES" ("CODIGO_EMPRESA", "CODIGO_ARTICULO", "DESCRIP_COMERCIAL", "CANTIDAD", "FOB_UNITARIO", "TOTAL_FOB", "FLETE", "TOTAL_CYF", "IMPUESTOS_ADUANALES", "SERVICIOS_ADUANAS", "ALMACENAJE_ADUANERO", "CARGADORES", "OTROS_GASTOS", "GASTOS_SEGURO", "UNIMED", "CAMARA_COMERCIO", "GRAVAMEN_ADUANAL", "COSTO_TOTAL", "COSTO_UNITARIO", "PRECIO_VENTA_ACTUAL", "COSTO_ANTERIOR", "NUMERO_EXPEDIENTE", "LOTE") AS 
  SELECT codigo_empresa,
         ...
         SUM (gravamen_aduanal) gravamen_aduanal,
           SUM (total_fob)
         + SUM (flete)
         + SUM (fletes_aereos)
         + SUM (fletes_maritimos)
    ...
    FROM (SELECT ar.codigo_empresa,
                 ...
                 DECODE (hma.codigo_movimiento, 'MCCCO', hma.importe_coste, 0)
                    camara_comercio,
                 DECODE (hma.codigo_movimiento, 'MCGRA', hma.importe_coste, 0)
                    gravamen_aduanal,
           ...
           WHERE                                 -- hma.tipo_movimiento = '09'
                ar   .codigo_empresa = hma.codigo_empresa
                 ...
                 AND hma.cantidad_unidad1 > 0  --AND ar.codigo_empresa = '004')                 
                                             )
